...
def predict_price(model, house_size):
    """Predicting house price for a given house size."""
    price_prediction = model.predict([[house_size]])[0]
    return price_prediction


def main():
    """Main function to run the complete analysis (training, evaluating, visualizing, and predicting) in a step-by-step manner."""
    print("="*60)
    ...
    y_pred, r2, rmse, mae = evaluate_model(model, X, y)
    
    # Creating visualization
    print("\n5. Creating visualization...")
    visualize_results(X, y, y_pred, model)
    
    # Example predictions for different house sizes (1500, 2000, 2500, 3000 sq ft)
    print("\n6. Example Predictions:")
    print("-" * 60)
    example_sizes = [1500, 2000, 2500, 3000]
    for size in example_sizes:
        predicted_price = predict_price(model, size)
        print(f"   House Size: {size:,} sq ft → Predicted Price: ${predicted_price:.2f}K (${predicted_price*1000:,.2f})")
...
if __name__ == "__main__":
    ...
    # Interactive prediction function (allowing users to predict prices for any house size)
    print("\n" + "="*60)
    print("INTERACTIVE PREDICTION")
    print("="*60)
    print("You can now use the model to predict prices for any house size.")
    print("Example usage:")
    print("  predicted_price = predict_price(model, 1800)")
    print("  print(f'Predicted price: ${predicted_price:.2f}K')")
    print("="*60)
